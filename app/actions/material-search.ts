...
export async function searchMaterialsAction(
  ...
  try {
    ...
    console.log('[SERVER ACTION DEBUG] Search parameters:', JSON.stringify({ query, page, limit }, null, 2));
    
    // Search materials in Supabase database
    console.log('[SERVER ACTION DEBUG] Searching materials in Supabase with query:', query);
    ...
    console.log(`[SERVER ACTION DEBUG] Search returned ${results.length} results out of ${count}`);
