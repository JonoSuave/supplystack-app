...
export const getFirecrawlConfig = () => ({
  ...
  baseUrl:
    ...
    "https://api.firecrawl.construction/v1",
});
...
export async function searchMaterials(
  ...
  while (retries <= MAX_RETRIES) {
    try {
      console.log(
        `[FIRECRAWL] Searching materials with params (attempt ${retries + 1}/${
          MAX_RETRIES + 1
        }):`,
      ...
      try {
        ...
        if (!response.ok) {
          ...
          console.warn(
            "[FIRECRAWL] Falling back to mock data due to API error"
          );
        ...
        console.log(
          `[FIRECRAWL] API response received with ${
      ...
      console.error(
        `[FIRECRAWL] Error searching materials (attempt ${retries + 1}/${
          MAX_RETRIES + 1
        }):`,
        error
      );
      ...
      if (retries < MAX_RETRIES) {
        ...
        console.warn(
          "[FIRECRAWL] Falling back to mock data after all retry attempts"
        );
        return getMockMaterialSearchResponse(params);
      }
    }
...
export async function getMaterialUpdates(
  ...
  while (retries <= MAX_RETRIES) {
    try {
      console.log(
        `[FIRECRAWL] Getting updates for materials (attempt ${retries + 1}/${
          MAX_RETRIES + 1
        }):`,
        materialIds
      );
      ...
      console.error(
        `[FIRECRAWL] Error getting material updates (attempt ${retries + 1}/${
          MAX_RETRIES + 1
        }):`,
        error
      );
...
export async function getMaterialDetails(
  ...
  while (retries <= MAX_RETRIES) {
    try {
      console.log(
        `[FIRECRAWL] Getting details for material (attempt ${retries + 1}/${
          MAX_RETRIES + 1
        }):`,
        materialId
      );
      ...
      const config = getFirecrawlConfig();
      const url = `${config.baseUrl}/materials/${encodeURIComponent(
        materialId
      )}`;
      ...
      try {
        ...
        const data = await response.json();
        console.log(
          `[FIRECRAWL] API response received for material: ${materialId}`
        );
      ...
      console.error(
        `[FIRECRAWL] Error getting material details (attempt ${retries + 1}/${
          MAX_RETRIES + 1
        }):`,
        error
      );
...
function getMockMaterialSearchResponse(
  params: MaterialSearchParams
): MaterialSearchResponse {
  console.log(
    "[FIRECRAWL DEBUG] Generating mock response for query:",
    params.query
  );
  ...
  const filteredResults = potentialResults.filter(
    (item) =>
  ...
  console.log(
    `[FIRECRAWL DEBUG] Found ${filteredResults.length} items matching query: ${searchQuery}`
  );
...
function getMockMaterialItem(id: string, query?: string): MaterialItem {
  ...
  if (
    query &&
    query.toLowerCase().includes("sand") &&
    query.toLowerCase().includes("paper")
  ) {
    materialName = `${selectedCategory === "Sandpaper" ? "" : "Sandpaper - "}${
      Math.floor(Math.random() * 400) + 60
    } Grit ${selectedCategory}`;
  } else if (query) {
  ...
  return {
    ...
    vendor: {
      ...
      contact: {
        ...
        email: `info@${randomVendor.name
          .toLowerCase()
          .replace(/[^a-z0-9]/g, "")}.com`,
        website: `https://www.${randomVendor.name
          .toLowerCase()
          .replace(/[^a-z0-9]/g, "")}.com`,
      },
    },
...
function getRandomAvailability():
  ...
  const statuses: ("in_stock" | "available_soon" | "special_order")[] = [
    "in_stock",
    "in_stock",
    "in_stock", // Higher probability for in_stock
    "available_soon",
    "available_soon",
    "special_order",
  ];
...
function getRandomLeadTime(): string {
  const leadTimes = ["1-2 days", "2-3 days", "3-5 days", "1 week", "2 weeks"];
...
function getSpecificationsForCategory(
  category: string
): Record<string, string> {
  switch (category) {
    case "Lumber":
      return {
        Dimensions: `${Math.floor(Math.random() * 10) + 1}" x ${
          Math.floor(Math.random() * 10) + 1
        }"`,
        Length: `${Math.floor(Math.random() * 12) + 8} ft`,
        "Wood Type": ["Pine", "Oak", "Cedar", "Maple"][
          Math.floor(Math.random() * 4)
        ],
        Treatment: ["Untreated", "Pressure-Treated", "Kiln-Dried"][
    ...
    case "Concrete":
      return {
        ...
        "Mix Type": ["Regular", "High-Strength", "Fast-Setting"][
          Math.floor(Math.random() * 3)
        ],
        Application: ["Foundation", "Slab", "Footings", "General Use"][
          Math.floor(Math.random() * 4)
        ],
      };
    case "Drywall":
      return {
        Thickness: `${
          [1 / 4, 3 / 8, 1 / 2, 5 / 8][Math.floor(Math.random() * 4)]
        }"`,
        Dimensions: `4' x ${[8, 10, 12][Math.floor(Math.random() * 3)]}'`,
        Type: ["Regular", "Moisture-Resistant", "Fire-Resistant"][
          Math.floor(Math.random() * 3)
        ],
      };
    default:
      return {
        Quality: ["Standard", "Premium", "Professional-Grade"][
          Math.floor(Math.random() * 3)
        ],
        Warranty: `${Math.floor(Math.random() * 10) + 1} years`,
        Certification: ["ASTM", "ANSI", "UL", "Energy Star"][
          Math.floor(Math.random() * 4)
        ],
      };
  }
